...
// Access generator for popup & screensaver
...
// AFFILIATE POOL FOR WAKE-UP TAX
const TAX_LINKS = [
...
// --- MATRIX RAIN CANVAS COMPONENT (PURE CODE SCREENSAVER) ---
const MatrixRain = () => {
    ...
    useEffect(() => {
        ...
        handleResize(); // Init
        window.addEventListener('resize', handleResize); // Handle Rotation
...
export default function App() {
  ...
  // MILK MONEY (VIRAL POPUP) STATE
  const [showViralPopup, setShowViralPopup] = useState(false);
  const [viralItem, setViralItem] = useState({ name: "", url: "" });

  // IDLE SCREENSAVER STATE (PARASITIC MONETIZATION)
  const [isIdle, setIsIdle] = useState(false);
  const [idleTimer, setIdleTimer] = useState<ReturnType<typeof setTimeout> | null>(null);
  // SECURITY CHECK STATE (RESUME TRAP)
  const [showSecurityCheck, setShowSecurityCheck] = useState(false);

  // EXIT INTENT STATE (DOOR SLAM)
  ...
  const resetIdleTimer = useCallback(() => {
    ...
    const newTimer = setTimeout(() => {
      setIsIdle(true);
    }, 180000); // 3 Minutes of inactivity triggers Screensaver
  ...
  useEffect(() => {
    ...
    window.addEventListener('touchstart', resetIdleTimer); // Mobile touch support
    resetIdleTimer(); // Init
    return () => {
  ...
  useEffect(() => {
      ...
      // Only attach on desktop (no mouseleave on mobile generally)
  ...
  const handleResume = (e: React.MouseEvent | React.TouchEvent) => {
      e.stopPropagation(); // Stop resetIdleTimer from firing instantly
      setIsIdle(false);
      resetIdleTimer();
      
      // THE WAKE-UP TAX: Open a revenue link in a new tab when they resume
      // This monetizes the "Click" action.
      ...
      // TRIGGER SECURITY CHECK (RESUME TRAP)
      setShowSecurityCheck(true);
      setTimeout(() => setShowSecurityCheck(false), 8000); // Hide after 8s
  };

  // --- VIRALITY: CAPTURE REFERRAL & POPUP TIMER ---
  useEffect(() => {
    ...
    const viralInterval = setInterval(() => {
        if (Math.random() > 0.7 && !showViralPopup && !isIdle && !showExitIntent) {
            ...
            // AUTO-DISMISS AFTER 10 SECONDS TO AVOID ANNOYANCE
            setTimeout(() => {
  ...
  // --- SUBLIMINAL SCREENSAVER (IDLE MONETIZATION) ---
  if (isIdle) {
    return (
      <div className="h-screen w-screen bg-black text-terminal-dim font-mono flex flex-col items-center justify-center p-8 relative overflow-hidden z-[100]" onClick={handleResume} onTouchStart={handleResume}>
        ...
        {/* PURE CANVAS MATRIX RAIN - INDESTRUCTIBLE */}
        <MatrixRain />
  ...
  return (
    // Use 100dvh for proper mobile height handling (ignores browser chrome)
    <div className={`flex flex-col h-[100dvh] font-mono selection:bg-terminal-green selection:text-terminal-black ${holidayMode ? 'bg-[#0a0000] text-red-100' : 'bg-terminal-black text-terminal-green'} relative overflow-hidden`}>
      ...
      {/* --- VIRAL NOTIFICATION (MILK MONEY) --- */}
      {/* Moved to top-left on mobile to avoid keyboard overlap */}
      {showViralPopup && (
      ...
      <header className={`flex items-center justify-between px-4 md:px-6 py-4 border-b z-10 relative shrink-0 ${holidayMode ? 'border-red-900 bg-[#0a0000]' : 'border-terminal-dim bg-terminal-black'}`}>
        ...
        {/* --- LIVE MARKET TICKER (HEADER EDITION) --- */}
      ...
      <div className="flex flex-1 overflow-hidden relative z-0 md:flex-row">
        ...
        {/* DESKTOP SIDEBAR (Static on Right) */}
